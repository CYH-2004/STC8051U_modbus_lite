...
| 功能 | 引脚 | 备注 |
| --- | --- | --- |
| UART4 | P0.2(RxD) P0.3(TxD) | 用于Modbus协议中的串口通信 |
| IIC | P1.5(SCL) P1.4(SDA) | 用于OLED显示屏的IIC通信 |
| 按键 | P3.2 | 按下时应为低电平 |

## 2026.3.10

移植初版协议至使用HAL库的STM32F401CCU6单片机，并已成功进行演示程序的测试。

注意由于STM32F401CCU6的UART6仅可使用PA12和PA11，但同时USB也使用这两个引脚，故连接USB线时会出现通讯失败的情况。

接线指南：

| 功能 | 引脚 | 备注 |
| --- | --- | --- |
| UART6 | PA12(RxD) PA11(TxD) | 用于Modbus协议中的串口通信 |
| IIC | PB6(SCL) PB7(SDA) | 用于OLED显示屏的IIC通信 |
| 按键 | PA1 | 按下时应为低电平 |
