...
uint8_t modbus_rx_standby;

uint8_t discrete_input_register[(DISCRETES_INPUT_MAX + 7) / 8];
uint8_t coil_register[(COILS_MAX + 7) / 8];
uint16_t input_register[INPUT_REG_MAX];
uint16_t holding_register[HOLDING_REG_MAX];

uint8_t RTU_TX_buffer[256];   //发送缓冲区

//modbus_RTU 初始化
uint8_t modbus_RTU_init(uint8_t mode, uint8_t addr);
...
void modbus_check_cplt(void);

//modbus_RTU 本地存储器访问
uint8_t *modbus_get_reg_addr(uint16_t reg_addr);
...
//modbus_RTU 从机程序
uint8_t modbus_RTU_slave_r1(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r2(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r3(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r4(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r5(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r6(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r15(uint8_t *buffer_p, uint8_t data_len);
uint8_t modbus_RTU_slave_r16(uint8_t *buffer_p, uint8_t data_len);
...
uint8_t modbus_RTU_init(uint8_t mode, uint8_t addr)
{
    ...
    HAL_TIM_Base_Start_IT(&htim11); //启动数据帧超时计时器
    HAL_UART_Receive_IT(&huart6, get_rx_buffer_addr(&huart6), 1);  //启动接收监听
...
{
    //清接收标志
    reset_rx_count(&huart6);
    modbus_rx_standby = 0;

    //重启接收
    HAL_UART_Receive_IT(&huart6, get_rx_buffer_addr(&huart6), 1);
}
...
uint8_t *modbus_get_reg_addr(uint16_t reg_addr)
{
    ...
    {
        return (uint8_t *)&input_register[(reg_addr - INPUT_REG_START_ADDRESS)];
    }
    else if((reg_addr >= HOLDING_REG_START_ADDRESS) && (reg_addr < HOLDING_REG_START_ADDRESS + HOLDING_REG_MAX))
    {
        return (uint8_t *)&holding_register[(reg_addr - HOLDING_REG_START_ADDRESS)];
    }
    return NULL;
}
...
uint8_t modbus_RTU_read_coils(uint8_t slave_addr, uint16_t reg_addr, uint16_t quantity, uint8_t *value, uint32_t timeout)
{
    ...
    HAL_UART_Transmit(&huart6, RTU_TX_buffer, 8, 100);
    ...
    while(wait_flag)
    {
        ...
        if(modbus_rx_standby)
        {
            //验证响应帧
            response_error_code = 0;
            RX_buffer_p_temp = get_rx_buffer_addr(&huart6);
            if(MODBUS_CRC16(RX_buffer_p_temp, get_rx_count(&huart6))) response_error_code = 0x59;
...
uint8_t modbus_RTU_read_discrete_input(uint8_t slave_addr, uint16_t reg_addr, uint16_t quantity, uint8_t *value, uint32_t timeout)
{
    ...
    while(wait_flag)
    {
        ...
        if(modbus_rx_standby)
        {
            ...
            response_error_code = 0;
            RX_buffer_p_temp = get_rx_buffer_addr(&huart6);
            crc = MODBUS_CRC16(RX_buffer_p_temp, get_rx_count(&huart6));
...
uint8_t modbus_RTU_read_holding_regs(uint8_t slave_addr, uint16_t reg_addr, uint16_t quantity, uint16_t *RX_value_p, uint32_t timeout)
{
    ...
    while(wait_flag)
    {
        ...
        if(modbus_rx_standby)
        {
            //验证响应帧
            response_error_code = 0;
            RX_buffer_p = get_rx_buffer_addr(&huart6);
            crc = MODBUS_CRC16(RX_buffer_p, get_rx_count(&huart6));
            if(crc) response_error_code = 0x59;
...
uint8_t modbus_RTU_write_single_coil(uint8_t slave_addr, uint16_t reg_addr, uint16_t value, uint32_t timeout)
{
    ...
    while(wait_flag)
    {
        ...
        if(modbus_rx_standby)
        {
            response_error_code = 0;
            //验证响应帧
            if(MODBUS_CRC16(get_rx_buffer_addr(&huart6), get_rx_count(&huart6)) == 0)	//CRC16校验，错误帧丢弃
            {
                response_error_code = !modbus_response_WR_check(RTU_TX_buffer, get_rx_buffer_addr(&huart6));   //内容检查
...
uint8_t modbus_RTU_write_multi_coils(uint8_t slave_addr, uint16_t reg_addr, uint16_t quantity, uint8_t *Value_p, uint32_t timeout)
{
    ...
    HAL_UART_Transmit(&huart6, RTU_TX_buffer, (uint8_t)(7 + 2 + RTU_TX_buffer[6]), 100);
...
uint8_t modbus_RTU_write_multi_regs(uint8_t slave_addr, uint16_t reg_addr, uint16_t quantity, uint16_t *Value_p, uint32_t timeout)
{
    ...
    HAL_UART_Transmit(&huart6, RTU_TX_buffer, (uint8_t)(7 + 2 + 2 * quantity), 100);
...
uint8_t modbus_RTU_slave_r1(uint8_t *buffer_p, uint8_t data_len)
{
    ...
    if(buffer_p[0] != 0)	//非广播地址则应答
    {
        ...
        HAL_UART_Transmit(&huart6, RTU_TX_buffer, (uint8_t)(5 + RTU_TX_buffer[2]), 100);
    }
    return 0;
}
...
void modbus_RTU_slave(void)
{
    ...
    if(modbus_rx_standby)
    {
        ...
        data_len = get_rx_count(&huart6);
        buffer_p = get_rx_buffer_addr(&huart6);
