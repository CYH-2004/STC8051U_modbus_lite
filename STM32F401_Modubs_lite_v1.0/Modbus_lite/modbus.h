...
extern uint8_t discrete_input_register[(DISCRETES_INPUT_MAX + 7) / 8];
extern uint8_t coil_register[(COILS_MAX + 7) / 8];
extern uint16_t input_register[INPUT_REG_MAX];
extern uint16_t holding_register[HOLDING_REG_MAX];

extern void modbus_check_cplt(void);

extern uint8_t modbus_RTU_init(uint8_t mode, uint8_t addr);   // modbus_RTU初始化
extern uint8_t modbus_mode(void);  //获取当前模式

extern uint8_t *modbus_get_reg_addr(uint16_t reg_addr);  //获取指定本地寄存器地址
