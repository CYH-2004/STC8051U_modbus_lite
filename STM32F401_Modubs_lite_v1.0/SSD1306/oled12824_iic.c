...
char WaveData[128];
    ...
    uint8_t OLED_GRAM[8][128];         // OLED全局缓存

#elif   OLED_GRAM_Mode == 1             //1:1024级缓存
    uint8_t OLED_GRAM[1024];           // OLED全局缓存
...
void delay_oled_ms(uint16_t ms)
{
    HAL_Delay(ms);
}


void IIC_Write_Byte(uint8_t slave,uint8_t addr,uint8_t *p,uint8_t number)       //IIC写入连续的几个字节
{
    HAL_I2C_Mem_Write(&hi2c1, slave, addr, 1, p, number, 10);
}
...
{
    OLED_Write_Byte((uint8_t)(0XB0 + y), OLED_CMD);  //设置Y坐标，查看手册输入0Xb0就是起始地址
#if     OLED_SIZE == 0      //0.96寸
...
void OLED_Clear(uint8_t mode)       //清屏函数
{
    ...
    for (i = 0; i<8; i++)       //循环8行
    {
        OLED_Write_Byte((uint8_t)(0xb0 + i), OLED_CMD);    //设置页地址（0~7）
        #if     OLED_SIZE == 0      //0.96寸
 ...
 */
void OLED_GRAM_Clear(uint8_t mode)       //通过缓冲数组，让OLED屏幕全亮全灭
{
    uint8_t x, y;
...
void OLED_DrawBlockG(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t mode)  //OLED画一个实心框
{
    ...
    {
        OLED_DrawLineG_Dot(x0, (uint8_t)(y0 + i * d), x1, (uint8_t)(y0 + i * d), mode, 1);
        OLED_Refresh_Gram();
    }
    // OLED_Refresh_Gram();
}
 ...
 * @note 注释：通过缓冲数组绘制，需要配合OLED_Refresh_Gram函数使用
 */
void OLED_DrawBox(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t fill, uint8_t mode)        //画空实心矩形
{
    ...
    if(fill == 0)  //空心
    {
        OLED_DrawLineG_Dot((uint8_t)xstart, (uint8_t)ystart, (uint8_t)xend, (uint8_t)ystart, mode, 1);
        OLED_DrawLineG_Dot((uint8_t)xstart, (uint8_t)yend, (uint8_t)xend, (uint8_t)yend, mode, 1);
        OLED_DrawLineG_Dot((uint8_t)xstart, (uint8_t)ystart, (uint8_t)xstart, (uint8_t)yend, mode, 1);
        OLED_DrawLineG_Dot((uint8_t)xend, (uint8_t)ystart, (uint8_t)xend, (uint8_t)yend, mode, 1);
    }
    else if (fill == 1) //实心
    {
        for ( i = ystart; i <= yend; i++)
        {
            OLED_DrawLineG_Dot((uint8_t)xstart, (uint8_t)i, (uint8_t)xend, (uint8_t)i, mode, 1);
        }
    }
}
 ...
 * @note 注释：通过缓冲数组绘制，需要配合OLED_Refresh_Gram函数使用
 */
void OLED_DrawCircleG(uint8_t x0, uint8_t y0, uint8_t R, uint8_t mode)      //OLED画一个圆形
{
...
void OLED_DrawCircleG_Distance(uint8_t x0,uint8_t y0, uint8_t R, uint8_t mode) //OLED画一个圆形（勾股定理法）
{
    ...
    while (x<=y)
    {
        OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 + y), mode);   //右下1
        OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 + y), mode);   //左下1
        OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 - y), mode);   //左上1
        OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 - y), mode);   //右上1
        OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 + x), mode);   //右下2
        OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 + x), mode);   //左下2
        OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 - x), mode);   //左上2
        OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 - x), mode);   //右上2
...
void OLED_DrawBlockCircleG_Distance(uint8_t x0,uint8_t y0, uint8_t R, uint8_t mode) //OLED画一个实心圆形（勾股定理法）
{
    ...
    while (x<=y)
    {
        // OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 + y), mode);
        // OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 + y), mode);
        OLED_DrawLineG_Dot((uint8_t)(x0 + x), (uint8_t)(y0 + y), (uint8_t)(x0 - x), (uint8_t)(y0 + y), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 - y), mode);
        // OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 - y), mode);
        OLED_DrawLineG_Dot((uint8_t)(x0 - x), (uint8_t)(y0 - y), (uint8_t)(x0 + x), (uint8_t)(y0 - y), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 + x), mode);
        // OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 + x), mode);
        OLED_DrawLineG_Dot((uint8_t)(x0 + y), (uint8_t)(y0 + x), (uint8_t)(x0 - y), (uint8_t)(y0 + x), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 - x), mode);
        // OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 - x), mode);
        OLED_DrawLineG_Dot((uint8_t)(x0 - y), (uint8_t)(y0 - x), (uint8_t)(x0 + y), (uint8_t)(y0 - x), mode, 1);
        x++;
 ...
 * @note 注释：通过缓冲数组绘制，需要配合OLED_Refresh_Gram函数使用
 */
void OLED_DrawCircleG_Bresenham(uint8_t x0, uint8_t y0, uint8_t R, uint8_t mode)      //OLED画一个圆形(Bresenham法)
{
    int x, y;
    int di;
    x = 0;
 ...
 * @note 注释：通过缓冲数组绘制，需要配合OLED_Refresh_Gram函数使用
 */
void OLED_DrawBlockCircleG_Bresenham(uint8_t x0, uint8_t y0, uint8_t R, uint8_t mode)      //OLED画一个实心圆形(Bresenham法)
{
    int x, y;
    int di;
    x = 0;
    ...
    while (x <= y)
    {
        // OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 + y), mode);        //6
        // OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 + y), mode);        //1
        OLED_DrawLineG_Dot((uint8_t)(x0 + x), (uint8_t)(y0 + y), (uint8_t)(x0 - x), (uint8_t)(y0 + y), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 - x), (uint8_t)(y0 - y), mode);        //2
        // OLED_DrawPoint((uint8_t)(x0 + x), (uint8_t)(y0 - y), mode);        //5
        OLED_DrawLineG_Dot((uint8_t)(x0 - x), (uint8_t)(y0 - y), (uint8_t)(x0 + x), (uint8_t)(y0 - y), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 + x), mode);        //4
        // OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 + x), mode);        //3
        OLED_DrawLineG_Dot((uint8_t)(x0 + y), (uint8_t)(y0 + x), (uint8_t)(x0 - y), (uint8_t)(y0 + x), mode, 1);
        // OLED_DrawPoint((uint8_t)(x0 - y), (uint8_t)(y0 - x), mode);        //7
        // OLED_DrawPoint((uint8_t)(x0 + y), (uint8_t)(y0 - x), mode);        //0
        OLED_DrawLineG_Dot((uint8_t)(x0 - y), (uint8_t)(y0 - x), (uint8_t)(x0 + y), (uint8_t)(y0 - x), mode, 1);
...
void OLED_DrawRoundRectangleG(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t R, uint8_t fill, uint8_t mode)      //OLED画一个圆角矩形(Bresenham法)
{
    ...
    if (R > (uint8_t)(xend - xstart) / 2 || R > (uint8_t)(yend - ystart) / 2)
        R = ((uint8_t)(xend - xstart) < (uint8_t)(yend - ystart)) ? ((uint8_t)(xend - xstart) / 2) : ((uint8_t)(yend - ystart) / 2);
    ...
    {   //空心
        while (x <= y)
        {
            OLED_DrawPoint((uint8_t)(xstart + R - x), (uint8_t)(ystart + R - y), mode);   //左上1
            OLED_DrawPoint((uint8_t)(xstart + R - y), (uint8_t)(ystart + R - x), mode);   //左上2
            OLED_DrawPoint((uint8_t)(xend - R + x), (uint8_t)(ystart + R - y), mode);   //右上1
            OLED_DrawPoint((uint8_t)(xend - R + y), (uint8_t)(ystart + R - x), mode);   //右上2
            OLED_DrawPoint((uint8_t)(xstart + R - x), (uint8_t)(yend - R + y), mode);   //左下1
            OLED_DrawPoint((uint8_t)(xstart + R - y), (uint8_t)(yend - R + x), mode);   //左下2
            OLED_DrawPoint((uint8_t)(xend - R + x), (uint8_t)(yend - R + y), mode);   //右下1
            OLED_DrawPoint((uint8_t)(xend - R + y), (uint8_t)(yend - R + x), mode);   //右下2
        ...
        OLED_DrawLineG_Dot((uint8_t)(xstart + R + 1), (uint8_t)ystart, (uint8_t)(xend - R - 1), (uint8_t)ystart, mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xstart + R + 1), (uint8_t)yend, (uint8_t)(xend - R - 1), (uint8_t)yend, mode, 1);
        OLED_DrawLineG_Dot((uint8_t)xstart, (uint8_t)(ystart + R + 1), (uint8_t)xstart, (uint8_t)(yend - R - 1), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)xend, (uint8_t)(ystart + R + 1), (uint8_t)xend, (uint8_t)(yend - R - 1), mode, 1);
    }
    else if( fill == 1)
    {   //实心
        while (x <= y)
        {
            OLED_DrawLineG_Dot((uint8_t)(xstart + R - x), (uint8_t)(ystart + R - y), (uint8_t)(xend - R + x), (uint8_t)(ystart + R - y), mode, 1);
            OLED_DrawLineG_Dot((uint8_t)(xstart + R - y), (uint8_t)(ystart + R - x), (uint8_t)(xend - R + y), (uint8_t)(ystart + R - x), mode, 1);
            OLED_DrawLineG_Dot((uint8_t)(xstart + R - x), (uint8_t)(yend - R + y), (uint8_t)(xend - R + x), (uint8_t)(yend - R + y), mode, 1);
            OLED_DrawLineG_Dot((uint8_t)(xstart + R - y), (uint8_t)(yend - R + x), (uint8_t)(xend - R + y), (uint8_t)(yend - R + x), mode, 1);
...
void OLED_DrawWidthRoundRectangleG(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, uint8_t R, uint8_t width, uint8_t mode)      //OLED画一个宽边圆角矩形(Bresenham法)
{
    ...
    while (x <= y)
    {
        OLED_DrawLineG_Dot((uint8_t)(xstart + R - x), (uint8_t)(ystart + R - y), (uint8_t)(xstart + R - x), (uint8_t)(ystart + R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xstart + R - y), (uint8_t)(ystart + R - x), (uint8_t)(xstart + R - y), (uint8_t)(ystart + R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xend - R + x), (uint8_t)(ystart + R - y), (uint8_t)(xend - R + x), (uint8_t)(ystart + R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xend - R + y), (uint8_t)(ystart + R - x), (uint8_t)(xend - R + y), (uint8_t)(ystart + R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xstart + R - x), (uint8_t)(yend - R + y), (uint8_t)(xstart + R - x), (uint8_t)(yend - R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xstart + R - y), (uint8_t)(yend - R + x), (uint8_t)(xstart + R - y), (uint8_t)(yend - R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xend - R + x), (uint8_t)(yend - R + y), (uint8_t)(xend - R + x), (uint8_t)(yend - R), mode, 1);
        OLED_DrawLineG_Dot((uint8_t)(xend - R + y), (uint8_t)(yend - R + x), (uint8_t)(xend - R + y), (uint8_t)(yend - R), mode, 1);
    ...
    OLED_DrawBox((uint8_t)(xstart + R + 1), (uint8_t)(ystart), (uint8_t)(xend - R - 1), (uint8_t)(ystart + width - 1), 1, mode);
    OLED_DrawBox((uint8_t)(xstart + R + 1), (uint8_t)(yend - width + 1), (uint8_t)(xend - R - 1), (uint8_t)(yend), 1, mode);
    OLED_DrawBox((uint8_t)(xstart), (uint8_t)(ystart + R + 1), (uint8_t)(xstart + width - 1), (uint8_t)(yend - R - 1), 1, mode);
    OLED_DrawBox((uint8_t)(xend - width + 1), (uint8_t)(ystart + R + 1), (uint8_t)(xend), (uint8_t)(yend - R - 1), 1, mode);
...
void OLED_DrawSin(void)     //画正弦函数
{
    uint8_t a = 8;   //X轴系数
    uint8_t b = 20;  //Y轴系数
    uint8_t x0 = 40, y0 = 32;    //起始点，结合后文，最好控制在中心点
    //x0 = 63 - 180/a
...
//         OLED_DrawLineG_Dot(Xtime, (uint8_t)(63 - WaveData[Xtime]), (uint8_t)(Xtime + 1), (uint8_t)(63 - WaveData[Xtime + 1]), 1, 1);
 ...
 * @note 注释：通过缓冲数组绘制，需要配合OLED_Refresh_Gram函数使用
 */
void OLED_ShowStringG(uint8_t x, uint8_t y, char *chr, uint8_t charSize, uint8_t Is_Reverse) //显示一个字符号串
{
    uint8_t j = 0;
...
void OLED_ShowNumG(uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t Size, uint8_t Is_Reverse)  //缓存显示一串数字
{
    ...
    for (t = 0; t < len; t++)
    {
        temp = (num / OLED_Index(10, (uint8_t)(len - t - 1))) % 10;      //取第t位数字
        if (enshow == 0 && t < (len - 1))       //如果数串长度小于指定长度
        {
            if (temp == 0)                      //多出来的位用空格补齐
            {
                OLED_ShowCharG((uint8_t)(x + 8 * t), y, ' ', Size, Is_Reverse);
                continue;                       //继续循环判断
            }
            else
                enshow = 1;                     //直到有数字为止
        }
        OLED_ShowCharG((uint8_t)(x + 8 * t), y, (uint8_t)(temp + '0'), Size, Is_Reverse);  //从高到低逐位显示数字
    }
}
...
//         {
//             if( y2>y1 )
//                 OLED_DrawPoint(x, (uint8_t)(y1 + (uint16_t)(y2 - y1) * (uint16_t)x / (uint16_t)(x2 - x1)), mode);
//             else
//                 OLED_DrawPoint(x, (uint8_t)(y1 - (uint16_t)(y1 - y2) * (uint16_t)x / (uint16_t)(x2 - x1)), mode);
//         }        
//     }
//     else
//     {
...
void OLED_ShowChar(uint8_t x, uint8_t y, uint8_t chr, uint8_t charSize, uint8_t Is_Reverse)  //早指定位置显示一个字符
{
    ...
    if (charSize == 16)                 //判断字号大小，读取F16X8字库，占2行
    {
        ...
        OLED_Set_Pos(x, (uint8_t)(y + 1));   //两行高度的下行
        for (i = 0; i<8; i++)
...
void OLED_ShowNum(uint8_t x, uint8_t y, uint32_t num, uint8_t len, uint8_t Size, uint8_t Is_Reverse)  //显示一串数字
{
    ...
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))       //如果数串长度小于指定长度
        {
            if (temp == 0)                      //多出来的位用空格补齐
            {
                OLED_ShowChar((uint8_t)(x + 8 * t), y, ' ', Size, Is_Reverse);
                continue;                       //继续循环判断
            }
            else
                enshow = 1;                     //直到有数字为止
        }
        OLED_ShowChar((uint8_t)(x + 8 * t), y, (uint8_t)(temp + '0'), Size, Is_Reverse);  //从高到低逐位显示数字
    }
}
...
void OLED_ShowChinese(uint8_t x, uint8_t y, uint8_t no, uint8_t Is_Reverse) //显示汉字
{
    ...
    OLED_Set_Pos(x, (uint8_t)(y + 1));   //汉字下半部分坐标（第x列，第y+1行）
    for (i = 0; i<16; i++)
    {
